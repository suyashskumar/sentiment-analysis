...
csv_path = "./sentiment_comments.csv"
...
def predict_sentiment(text, model, vectorizer):
    try:
        # If the input is a single comment
        if isinstance(text, str):
            text_tfidf = vectorizer.transform([text])
            sentiment = model.predict(text_tfidf)[0]
            print(f"Predicted sentiment for '{text}': {sentiment}")  # Debugging line
            return sentiment

        # If the input is a list of comments (for bulk prediction)
        elif isinstance(text, list):
            sentiments = []
            for single_text in text:
                text_tfidf = vectorizer.transform([single_text])
                sentiment = model.predict(text_tfidf)[0]
                print(f"Predicted sentiment for '{single_text}': {sentiment}")  # Debugging line
